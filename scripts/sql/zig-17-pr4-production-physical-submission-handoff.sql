-- ZIG-17 PR 4 PRODUCTION SQL — FOR LOGAN MANUAL REVIEW AND EXECUTION ONLY
...
--
-- Tracked schema assumptions (verified against ZIG-8/ZIG-11 migrations):
--   job_actions.status is text (not enum)
--   automation_runs.run_type is text
--   automation_runs.run_status may be enum-backed or text (compare via ::text)
--   jobs.job_status may be enum-backed or text (compare/assign via text literals)
--   completed actions may have completed_by NULL (do not backfill)
--
-- PREFLIGHT (read-only; run before the transaction if desired):
...
--   GROUP BY job_id
--   HAVING count(*) > 1;
--   Expect 0 rows. If any appear → STOP. No automatic dedupe.
