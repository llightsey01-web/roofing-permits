/**
 * Sarasota County Accela runner — peer of Hillsborough/Pinellas/Pasco (Angular login).
 *
 * workflow_file: sarasota-county.runner.js
 * Portal: https://aca-prod.accela.com/SARASOTACO
 *
 ...
 * No polk-county.runner.js changes required (peer runType IDs already include sarasota-county).
 */

const sarasotaConfig = require('./configs/sarasota-county.config')
...
async function loginSarasotaAngularCommunityView(page, credentials, config, companyId) {
  var selectors = config.selectors
  var sessionProvider = config.sessionProvider || 'sarasota_accela'
  ...
  if (sessionOk) {
    console.log('[sarasota_accela] Using saved session — skipping login ✓')
    return
  }

  console.log('[sarasota_accela] Session expired or missing — logging in fresh')
  ...
  if (!frame) {
    throw Object.assign(
      new Error('Sarasota Angular CommunityView login iframe not found'),
  ...
  console.log('[sarasota_accela] Login complete — session will be saved for next run')
}

async function resolveSarasotaLegalDescription(page, parcelNumber, selectors) {
  return readLegalDescriptionFromPortal(page, selectors)
}

function sarasotaHooks() {
  return {
    resolveLegalDescription: resolveSarasotaLegalDescription,
    performLogin: loginSarasotaAngularCommunityView,
  }
}

async function createSarasotaApplication(jobData, runId, runnerOptions) {
...
async function uploadSarasotaDocuments(jobData, runId, runnerOptions) {
  return runAccelaPortal(
    jobData,
    runId,
...
async function submitSarasotaApplication(_jobData, _runId, _runnerOptions) {
  throw Object.assign(
    new Error(
      'Sarasota permit_submit is disabled; Accela submit/payment requires human approval ' +
...
async function runSarasotaCounty(jobData, runId, runnerOptions) {
  var opts = runnerOptions || {}
  var runType = opts.runType || 'permit_phase_1'

  console.log('[sarasota] Starting', {
    runId: runId,
    jobId: jobData && jobData.id,
    runType: runType,
    portalUrl: sarasotaConfig.portalUrl,
  })

  if (runType === 'permit_submit') {
    return submitSarasotaApplication(jobData, runId, opts)
  }

  await loadCredentials(jobData.company_id, jobData.ahj_id)

  if (runType === 'permit_document_upload') {
    return uploadSarasotaDocuments(jobData, runId, opts)
  }
  if (runType === 'permit_resume') {
    return runAccelaPortal(jobData, runId, opts, sarasotaConfig, sarasotaHooks())
  }
  return createSarasotaApplication(jobData, runId, opts)
}

module.exports = {
  runSarasotaCounty,
  loginSarasotaAngularCommunityView,
  createSarasotaApplication,
  uploadSarasotaDocuments,
  submitSarasotaApplication,
  sarasotaConfig,
}
