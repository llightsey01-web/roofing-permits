/**
 * Citrus County Accela runner — Angular CommunityView peer.
 *
 * workflow_file: citrus-county.runner.js
 * Portal: https://aca-prod.accela.com/CITRUS (agency_code=CITRUS)
 *
...
const citrusConfig = require('./configs/citrus-county.config')
...
async function loginCitrusAngularCommunityView(page, credentials, config, companyId) {
  var selectors = config.selectors
  var sessionProvider = config.sessionProvider || 'citrus_accela'
  ...
  if (sessionOk) {
    console.log('[citrus_accela] Using saved session — skipping login ✓')
    return
  }

  console.log('[citrus_accela] Session expired or missing — logging in fresh')
  ...
  if (!frame) {
    throw Object.assign(
      new Error('Citrus Angular CommunityView login iframe not found'),
  ...
  console.log('[citrus_accela] Login complete — session will be saved for next run')
}

async function resolveCitrusLegalDescription(page, parcelNumber, selectors) {
  return readLegalDescriptionFromPortal(page, selectors)
}

function citrusHooks() {
  return {
    resolveLegalDescription: resolveCitrusLegalDescription,
    performLogin: loginCitrusAngularCommunityView,
  }
}

async function createCitrusApplication(jobData, runId, runnerOptions) {
...
async function uploadCitrusDocuments(jobData, runId, runnerOptions) {
  return runAccelaPortal(
    jobData,
    runId,
...
async function submitCitrusApplication(_jobData, _runId, _runnerOptions) {
  throw Object.assign(
    new Error(
      'Citrus permit_submit is disabled; Accela submit/payment requires human approval ' +
...
async function runCitrusCounty(jobData, runId, runnerOptions) {
  var opts = runnerOptions || {}
  var runType = opts.runType || 'permit_phase_1'

  console.log('[citrus] Starting', {
    runId: runId,
    jobId: jobData && jobData.id,
    runType: runType,
    portalUrl: citrusConfig.portalUrl,
    agencyCode: 'CITRUS',
  })

  if (runType === 'permit_submit') {
    return submitCitrusApplication(jobData, runId, opts)
  }

  await loadCredentials(jobData.company_id, jobData.ahj_id)

  if (runType === 'permit_document_upload') {
    return uploadCitrusDocuments(jobData, runId, opts)
  }
  if (runType === 'permit_resume') {
    return runAccelaPortal(jobData, runId, opts, citrusConfig, citrusHooks())
  }
  return createCitrusApplication(jobData, runId, opts)
}

module.exports = {
  runCitrusCounty,
  loginCitrusAngularCommunityView,
  createCitrusApplication,
  uploadCitrusDocuments,
  submitCitrusApplication,
  citrusConfig,
}
