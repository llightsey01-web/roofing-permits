/**
 * Brevard County Accela runner — Angular CommunityView peer (BASS).
 *
 * workflow_file: brevard-county.runner.js
 * Portal: https://aca-prod.accela.com/BREVARD (agency_code=BREVARD)
 *
...
const brevardConfig = require('./configs/brevard-county.config')
...
async function loginBrevardAngularCommunityView(page, credentials, config, companyId) {
  var selectors = config.selectors
  var sessionProvider = config.sessionProvider || 'brevard_accela'
  ...
  if (sessionOk) {
    console.log('[brevard_accela] Using saved session — skipping login ✓')
    return
  }

  console.log('[brevard_accela] Session expired or missing — logging in fresh')
  ...
  if (!frame) {
    throw Object.assign(
      new Error('Brevard Angular CommunityView login iframe not found'),
  ...
  console.log('[brevard_accela] Login complete — session will be saved for next run')
}

async function resolveBrevardLegalDescription(page, parcelNumber, selectors) {
  return readLegalDescriptionFromPortal(page, selectors)
}

function brevardHooks() {
  return {
    resolveLegalDescription: resolveBrevardLegalDescription,
    performLogin: loginBrevardAngularCommunityView,
  }
}

async function createBrevardApplication(jobData, runId, runnerOptions) {
...
async function uploadBrevardDocuments(jobData, runId, runnerOptions) {
  return runAccelaPortal(
    jobData,
    runId,
...
async function submitBrevardApplication(_jobData, _runId, _runnerOptions) {
  throw Object.assign(
    new Error(
      'Brevard permit_submit is disabled; Accela submit/payment requires human approval ' +
...
async function runBrevardCounty(jobData, runId, runnerOptions) {
  var opts = runnerOptions || {}
  var runType = opts.runType || 'permit_phase_1'

  console.log('[brevard] Starting', {
    runId: runId,
    jobId: jobData && jobData.id,
    runType: runType,
    portalUrl: brevardConfig.portalUrl,
    agencyCode: 'BREVARD',
  })

  if (runType === 'permit_submit') {
    return submitBrevardApplication(jobData, runId, opts)
  }

  await loadCredentials(jobData.company_id, jobData.ahj_id)

  if (runType === 'permit_document_upload') {
    return uploadBrevardDocuments(jobData, runId, opts)
  }
  if (runType === 'permit_resume') {
    return runAccelaPortal(jobData, runId, opts, brevardConfig, brevardHooks())
  }
  return createBrevardApplication(jobData, runId, opts)
}

module.exports = {
  runBrevardCounty,
  loginBrevardAngularCommunityView,
  createBrevardApplication,
  uploadBrevardDocuments,
  submitBrevardApplication,
  brevardConfig,
}
