...
const {
  ...
  isTerminalRunStatus,
} = require('../automation/run-status.js')

const POLL_INTERVAL_MS = 30000
const STALE_MS = 10 * 60 * 1000
const SUCCESS_STATUSES = SUCCESS_READ_STATUSES
...
const RUN_TYPE_GROUPS = [
  ...
  { key: 'permit_portal', label: 'Permit Portal', types: ['permit_phase_1', 'permit_resume', 'permit_submit', 'permit_document_upload'] },
...
function summarizeRunOutcomes(runs) {
  ...
  ;(runs || []).forEach(function (run) {
    var status = run && run.run_status
    if (!isTerminalRunStatus(status)) return
    total += 1
    if (isSuccessfulRunStatus(status)) success += 1
    else if (isInterventionRunStatus(status)) intervention += 1
    else if (isFailedRunStatus(status)) failure += 1
    else if (isCancelledRunStatus(status)) cancelled += 1
  })
...
async function computeSystemMetrics(supabase) {
  ...
  var captchaTotal = 0
  var captchaFail = 0
  var overall = summarizeRunOutcomes(runs30)

  runs30.forEach(function (run) {
    if (!isTerminalRunStatus(run.run_status)) return
    ...
    if (err.indexOf('captcha') !== -1 || (run.run_type || '').indexOf('captcha') !== -1) {
      captchaTotal += 1
      if (isFailedRunStatus(run.run_status)) captchaFail += 1
    }
  })
  ...
  var overallPct = overall.total > 0 ? Math.round((overall.success / overall.total) * 1000) / 10 : null
  ...
  return {
    ...
    success_rates: {
      ...
      overall: {
        ...
        total: overall.total,
        intervention: overall.intervention,
...
async function upsertDailyPlatformMetrics(supabase, options) {
  ...
  var outcomes = summarizeRunOutcomes(runList)
  var success = outcomes.success
  var total = outcomes.total
  var captchaFail = 0
  var captchaTotal = 0
  var byType = {}
  ...
  var rows = [
    ...
    },
    {
      ...
      metadata: {
        success: success,
        total: total,
        intervention: outcomes.intervention,
