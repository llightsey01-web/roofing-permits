...
// Outcome classifier for persisted run_status. Reader-only.
// ACTIVE folds queued + running. UNKNOWN is fail-closed.
var RUN_OUTCOME = Object.freeze({
  SUCCESS: 'success',
  INTERVENTION: 'intervention',
  FAILURE: 'failure',
  ACTIVE: 'active',
...
function classifyRunOutcome(status) {
  if (isSuccessfulRunStatus(status)) return RUN_OUTCOME.SUCCESS
  if (isInterventionRunStatus(status)) return RUN_OUTCOME.INTERVENTION
  if (isFailedRunStatus(status)) return RUN_OUTCOME.FAILURE
  if (isCancelledRunStatus(status)) return RUN_OUTCOME.CANCELLED
  if (isActiveRunStatus(status)) return RUN_OUTCOME.ACTIVE
  return RUN_OUTCOME.UNKNOWN
}
...
module.exports = {
  ...
  classifyRunOutcome: classifyRunOutcome,
