...
// ZIG-10: validate AHJ packet configuration before the skeleton RPC.
// No PDF fill, no packet assembly, no job_documents completeness checks.
 ...
 * Throws packet_config_missing before any handoff RPC.
 *
...
/**
 * Stub permit_packet run: validate packet config, atomic ready_for_physical_submission
 * + pending action, then mark the automation_run terminal (no Playwright).
 *
 * @param {object} supabase
 * @param {object} job
...
async function runPermitPacketSkeleton(supabase, job, runRecord) {
  ...
  // ZIG-10: fail closed on missing/invalid AHJ packet configuration before handoff.
  await assertPacketConfigForJob(supabase, job)
  ...
  if (runId) {
    ...
    var update = await supabase
      ...
      .update({
        ...
        checkpoint_data: {
          zig8_skeleton: true,
          zig10_packet_config_validated: true,
