...
async function loadPostPhase1Chain() {
  var mod = await import('./noc-proof-erecord-chain.js')
...
async function triggerNocAfterPhase1Direct(jobId, options) {
  ...
  var chain = await loadPostPhase1Chain()
  var result = await chain.runPostPhase1Chain(jobId, options || {})
