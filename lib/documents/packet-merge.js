...
/**
 * Deterministic pdf-lib page-copy merge. Caller supplies buffers in
 * configured requirement order. Does not inspect storage listings.
 * @param {Array<Buffer|Uint8Array>} orderedBytes
 * @returns {Promise<Buffer>}
 */
async function mergePdfBuffers(orderedBytes) {
  var buffers = orderedBytes || []
  var mergedPdf = await PDFDocument.create()
  for (var i = 0; i < buffers.length; i++) {
    var src = await PDFDocument.load(buffers[i])
...
async function maybeMergeCombinedPacket(supabase, job, options) {
  ...
  for (var i = 0; i < ordered.length; i++) {
    ...
    orderedBytes.push(Buffer.from(await download.data.arrayBuffer()))
  }
