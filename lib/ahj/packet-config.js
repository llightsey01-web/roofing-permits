...
/**
 * Non-throwing packet readiness for admin dashboard / ops surfaces.
 * Reuses loadPacketRequirements + isPacketConfigValid — does not duplicate rules.
 *
 * @param {object} supabase
 * @param {string} ahjId
 * @param {string|null|undefined} workflowType
 * @returns {Promise<{ status: 'ready'|'missing'|'invalid'|'not_applicable', reason?: string }>}
 */
async function getPacketReadiness(supabase, ahjId, workflowType) {
  if (workflowType !== 'pdf_packet') {
    return { status: 'not_applicable' }
  }

  try {
    var rows = await loadPacketRequirements(supabase, ahjId)
    var validation = isPacketConfigValid(rows)
    if (validation.valid) {
      return { status: 'ready' }
    }
    return {
      status: 'invalid',
      reason: validation.reason || 'packet configuration is invalid',
    }
  } catch (err) {
    if (err && err.errorCode === 'packet_config_missing') {
      return {
        status: 'missing',
        reason: 'packet_config_missing',
      }
    }
    return {
      status: 'invalid',
      reason: 'packet readiness evaluation failed',
    }
  }
}

/**
 * Pure evaluation over already-loaded requirement rows (batch dashboard path).
 * @param {object[]|null|undefined} requirements
 * @param {string|null|undefined} workflowType
 * @returns {{ status: 'ready'|'missing'|'invalid'|'not_applicable', reason?: string }}
 */
function evaluatePacketReadinessFromRows(requirements, workflowType) {
  if (workflowType !== 'pdf_packet') {
    return { status: 'not_applicable' }
  }
  var rows = requirements || []
  if (!rows.length) {
    return { status: 'missing', reason: 'packet_config_missing' }
  }
  var validation = isPacketConfigValid(rows)
  if (validation.valid) {
    return { status: 'ready' }
...
module.exports = {
  ...
  getPacketReadiness,
  evaluatePacketReadinessFromRows,
}
