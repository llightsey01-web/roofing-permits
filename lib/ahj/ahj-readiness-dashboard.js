...
// Reader contract: proven success is complete (+ historical completed).
// needs_review is intervention and must not raise the success/readiness score.
var SUCCESS_STATUSES = new Set(SUCCESS_READ_STATUSES.slice())
var FAILURE_STATUSES = new Set(FAILURE_READ_STATUSES.slice())
...
function aggregateRunMetrics(runs) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    if (isSuccessfulRunStatus(status)) {
      ...
      streakActive = false
    } else if (isFailedRunStatus(status)) {
    ...
    } else if (isInterventionRunStatus(status)) {
      metrics.intervention_count += 1
      if (!metrics.last_intervention_at) metrics.last_intervention_at = at
      // Terminal non-success: does not prove automation and does not extend
      // a failure streak.
      streakActive = false
    } else if (isCancelledRunStatus(status) || isActiveRunStatus(status)) {
      streakActive = false
    } else {
      streakActive = false
