...
async function verifyAndAddProduct(input) {
  ...
  try {
    ...
    if (
      /There are no records that match the search criteria/i.test(listText) ||
      /No applications found|No records|0 record/i.test(listText)
    ) {
      await browser.close()
      return {
        valid: false,
        error:
          'FL approval number not found on Florida Building Commission. Please verify at floridabuilding.org',
      }
    }
