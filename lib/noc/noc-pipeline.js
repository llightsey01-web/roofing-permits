...
const { NOC_FIELDS } = require('./noc-field-map')
const { persistGeneratedNocDocument } = require('../documents/upsert-canonical-job-document')
...
async function fillNocForm(pdfDoc, job, company) {
  ...
  safeSetField(form, NOC_FIELDS.PERMIT_NUMBER, job.permit_number || job.ahj_permit_number || '')

  safeSetField(form, NOC_FIELDS.TAX_FOLIO_NUMBER, job.parcel_number || '')
  // Long legal / address / contractor lines: auto-shrink (were hard-clipping at Helv 10)
  safeSetFieldAutoFit(
    form,
    NOC_FIELDS.LEGAL_DESCRIPTION,
    legalDescription || streetAddress
  )
  safeSetFieldAutoFit(form, NOC_FIELDS.STREET_ADDRESS, streetAddress)
  safeSetFieldAutoFit(form, NOC_FIELDS.GENERAL_DESCRIPTION, generalDescription)

  safeSetFieldAutoFit(
    form,
    NOC_FIELDS.OWNER_NAME_AND_ADDRESS,
    (job.owner_name || '') + (fullAddress ? ', ' + fullAddress : '')
  )
  safeSetField(form, NOC_FIELDS.OWNER_INTEREST, 'Fee Simple')
  // fee simple titleholder if other than owner — no separate data source; leave blank

  safeSetFieldAutoFit(form, NOC_FIELDS.CONTRACTOR_NAME_AND_ADDRESS, contractorAddress)
  safeSetField(form, NOC_FIELDS.CONTRACTOR_PHONE, company && company.phone ? company.phone : '')
  ...
  safeUncheck(form, NOC_FIELDS.NOTARY_PHYSICAL_PRESENCE_CHECKBOX)
  safeCheck(form, NOC_FIELDS.NOTARY_ONLINE_CHECKBOX)

  var generalDescriptionInForm = generalDescription
  try {
    generalDescriptionInForm = form.getTextField(NOC_FIELDS.GENERAL_DESCRIPTION).getText()
  } catch (e) {}
...
async function generateNOC(jobId, job, company, options) {
  ...
  if (!skipUpload) {
    ...
    await persistGeneratedNocDocument(supabase, jobId, filePath, pdfBytes.length)
