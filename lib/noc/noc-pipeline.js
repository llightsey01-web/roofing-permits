...
async function generateNOC(jobId, job, company) {
  ...
  // County from AHJ portal
  let county = ''
  if (job.ahj_id) {
    try {
      const { data: ahj } = await supabase
        .from('ahj_portals')
        .select('county_or_city, state')
        .eq('id', job.ahj_id)
        .single()
  ...
  // Qualifier info from company
  safeSetField(form, 'Qualifier Name', company?.qualifier_name || '')
  safeSetField(form, 'Qualifier License', company?.qualifier_license || '')

  // Lender / bond when present on job
  if (job.lender_name || job.lender_address) {
    const lenderLine = [job.lender_name, job.lender_address].filter(Boolean).join(', ')
    safeSetField(form, 'Name and Address', lenderLine)
  }
  safeSetField(form, 'Lenders phone number', job.lender_phone || '')
  safeSetField(form, 'Amount of bond', job.bond_amount || job.amount_of_bond || '')
  if (job.surety_name || job.surety_address) {
    const suretyLine = [job.surety_name, job.surety_address].filter(Boolean).join(', ')
    safeSetField(form, 'Name and address_2', suretyLine)
  }
