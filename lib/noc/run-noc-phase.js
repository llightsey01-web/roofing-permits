// lib/noc/run-noc-phase.js
// NOC validation + PDF pipeline only (no Proof/eRecord chain — safe for Next API routes)

const { createClient } = require('@supabase/supabase-js')
const ws = require('ws')
const { startNOCPipeline } = require('./noc-pipeline.js')
...
async function runNocPhaseForJob(jobId, options) {
  if (!jobId) {
  ...
  if (pipelineResult && pipelineResult.needsManualReview) {
    return {
      success: false,
      ...
      overflows: pipelineResult.overflows,
