...
/**
 * Draft monthly invoice generation from confirmed vendor_payments.
 ...
 * permit_fees_total_cents = sum of confirmed vendor_payments in [periodStart, periodEnd).
 ...
 * @param {number} [input.subscriptionAmountCents=0] — must stay 0 until pricing exists unless explicitly overridden for tests
...
async function createDraftInvoice(input) {
  ...
  var feeTotal = await getCompanyVendorCostTotalForPeriod(companyId, periodStart, periodEnd, {
    currency: currency,
  })
