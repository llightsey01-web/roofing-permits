...
async function uploadPdfToNewTransaction(page, pdfPath) {
  ...
  var fileInput = await page.waitForSelector('input[type="file"]', { timeout: 60000 })
...
async function withProofSession(pdfBytes, handler, options) {
  ...
  try {
    return await withSession('proof', companyId, browser, async function (page) {
      page.setDefaultTimeout(120000)
      await page.goto(proofConfig.loginUrl, { waitUntil: 'domcontentloaded' })
      await page.waitForTimeout(1500)
      var valid = await isSessionValid(page)
      if (!valid) {
        console.log('[proof] Session expired — logging in fresh')
        await clearSession('proof', companyId)
        await login(page, { companyId: companyId })
        console.log('[proof] Login complete — session saved for next run')
      } else {
        console.log('[proof] Using saved session — skipping login ✓')
      }
...
async function sendForNotarizationWithFallback(opts) {
  ...
  if (attempts >= 3) {
    // DocuSign SDK must not be imported from the Next.js API graph (breaks Turbopack builds).
    // Workers should call lib/docusign/notarization-fallback.js directly after Proof exhaustion.
    throw new Error(
      'Proof.com failed 3 times — invoke DocuSign via lib/docusign/notarization-fallback.js from the worker'
    )
  }
  ...
  try {
    ...
    return await sendForNotarizationWithFallback(Object.assign({}, options, { attempts: attempts + 1 }))
  }
}
