...
async function login(page) {
  ...
  await (await page.waitForSelector(proofConfig.selectors.loginEmail, { timeout: 10000 })).fill(process.env.PROOF_EMAIL)
  await page.waitForTimeout(400)
  await page.keyboard.press('Enter')
  await page.waitForTimeout(2500)
  await (await page.waitForSelector(proofConfig.selectors.loginPassword, { timeout: 10000 })).fill(process.env.PROOF_PASSWORD)
  await page.waitForTimeout(400)
  await page.keyboard.press('Enter')
...
async function withProofSession(pdfBytes, handler, options) {
  ...
  var browser = await chromium.launch({
    headless: true,
    slowMo: (options && options.slowMo) || 500,
  })
  var page = await browser.newPage()
  page.setDefaultTimeout(45000)
  var uploadFilename = (options && options.uploadFilename) || ('noc-proof-' + Date.now() + '.pdf')
  ...
  try {
    ...
    if (options && options.identity) {
      await configureProofTransactionIdentity(page, options.identity)
    }
