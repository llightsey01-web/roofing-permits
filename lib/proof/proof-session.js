...
async function sendForNotarizationWithFallback(opts) {
  ...
  if (attempts >= 3) {
    ...
    // Load outside Next/Turbopack static analysis — DocuSign SDK is worker/API-only
    var docusignPath = require('path').join(__dirname, '..', 'docusign', 'docusign-session.js')
    var docusignSession = require(docusignPath)
    return await docusignSession.sendForNotarization(options)
  }
