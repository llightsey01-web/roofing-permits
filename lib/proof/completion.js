...
async function checkProofCompletionForJob(page, job, options) {
  ...
  var localPdfPath = join(outputDir || process.cwd(), 'noc-notarized.pdf')
  ...
  try {
    var { continueToErecordPrep } = require('../automation/noc-proof-erecord-chain')
    erecordPrepResult = await continueToErecordPrep(job.id, {
  ...
  return {
    ...
    jobId: job.id,
    transactionId: transactionId,
    nocStatus: erecordPrepResult ? 'ready_for_erecord_review' : 'queued_for_erecord',
...
async function runProofCompletionCheck(options) {
  ...
  var outputDir = opts.outputDir || join('automation', 'logs', 'proof-completion-' + Date.now())
