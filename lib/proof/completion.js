...
const { persistNotarizedNocDocument } = require('../documents/upsert-canonical-job-document')
...
async function checkProofCompletionForJob(page, job, options) {
  ...
  var notarizedBytes = readFileSync(localPdfPath)
  await persistNotarizedNocDocument(supabase, job.id, storagePath, notarizedBytes.length)
  notarizedBytes = null
  ...
  try {
    var chainFns = require('../automation/noc-proof-erecord-chain.js')
