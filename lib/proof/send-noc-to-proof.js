...
function getApprovedProofTestEmails() {
  return String(process.env.PROOF_APPROVED_TEST_EMAILS || process.env.PROOF_TEST_EMAIL || '')
    .split(',')
    .map(function(s) { return s.trim().toLowerCase() })
    .filter(Boolean)
}

function isApprovedProofTestEmail(email) {
  var approved = getApprovedProofTestEmails()
  if (!approved.length) return false
  return approved.indexOf(String(email || '').trim().toLowerCase()) >= 0
}
...
function evaluateProofSendGate(job) {
  ...
  } else if (!isApprovedProofTestEmail(job.owner_email)) {
    reasons.push('owner_email not in PROOF_APPROVED_TEST_EMAILS (got ' + job.owner_email + ')')
  }
  ...
  if (credentialError) reasons.push(credentialError)

  return {
    allowed: reasons.length === 0,
    reasons: reasons,
    owner_email: job.owner_email || null,
    approved_emails_configured: getApprovedProofTestEmails().length > 0,
  }
}

function validateProofCredentials() {
  if (!process.env.PROOF_EMAIL || !String(process.env.PROOF_EMAIL).trim()) {
    return 'Proof credentials missing — set PROOF_EMAIL in .env.local'
  }
  if (!process.env.PROOF_PASSWORD || !String(process.env.PROOF_PASSWORD).trim()) {
    return 'Proof credentials missing — set PROOF_PASSWORD in .env.local'
  }
  return null
}
...
module.exports = {
  ...
  getApprovedProofTestEmails,
  isApprovedProofTestEmail,
