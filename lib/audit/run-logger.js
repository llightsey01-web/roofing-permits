...
async function captureFailureForensics(opts) {
  ...
  // Mirror into durable workflow artifacts when this run is linked
  try {
    var workflowRunId =
      (options.metadata && options.metadata.workflow_run_id) ||
      options.workflowRunId ||
      null
    var workflowStepId =
      (options.metadata && options.metadata.workflow_step_id) ||
      options.workflowStepId ||
      null

    if (!workflowRunId && runId && supabase) {
      ...
      if (legacy && legacy.payload) {
        workflowRunId = legacy.payload.workflow_run_id || null
        workflowStepId = workflowStepId || legacy.payload.workflow_step_id || null
      }
    }

    if (workflowRunId || runId) {
      var { getObservability } = require('../workflow/observability.js')
      var obs = getObservability()
      await obs.mirrorLegacyCapture({
        legacyRunId: runId,
        ...
        htmlBucket: 'job-documents',
        success: false,
        url: currentUrl,
        source: 'run_logger_forensics',
      })
    }
  } catch (obsErr) {
    console.warn('[forensics] workflow mirror failed:', obsErr.message)
  }
