...
async function loadCrypto() {
  return import('../crypto/credential-encryption.js')
}

async function decryptField(value) {
  if (!value) return null
  var crypto = await loadCrypto()
  return crypto.decryptCredential(value)
}

async function decryptExtra(extra) {
  if (!extra || typeof extra !== 'object') return null
  var crypto = await loadCrypto()
  var out = {}
  for (var key of Object.keys(extra)) {
    if (extra[key]) out[key] = crypto.decryptCredential(String(extra[key]))
  }
  return out
}
...
async function fetchLegacyAhjCredential(companyId, ahjId) {
  ...
  if (!password && data.password_encrypted) {
    password = await decryptField(data.password_encrypted)
  }
...
async function mapVaultRowToCredentials(provider, row) {
  ...
  var username = row.encrypted_username ? await decryptField(row.encrypted_username) : null
  var password = row.encrypted_password ? await decryptField(row.encrypted_password) : null
  var extra = row.encrypted_extra ? await decryptExtra(row.encrypted_extra) : null
