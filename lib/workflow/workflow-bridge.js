...
function createWorkflowBridge(options) {
  ...
  var runStatus = require('../automation/run-status.js')
  ...
  async function syncLegacyRunCompletion(input) {
    ...
    if (activityId) {
      var activityStatus = runStatus.mapRunStatusToWorkflowActivityStatus(legacy.run_status)
      ...
      await state.supabase
        ...
        .update({
          ...
          result: {
            ...
            outcome: runStatus.classifyRunOutcome(legacy.run_status),
          },
          error_message: legacy.error_message || null,
          completed_at: runStatus.isTerminalRunStatus(legacy.run_status)
