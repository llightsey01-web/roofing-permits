...
var { classifyRunOutcome, RUN_OUTCOME } = require('../automation/run-status.js')
...
/**
 * After legacy erecord_prepare / erecord_submit persists, sync workflow from
 * the stored automation_runs row. Caller-provided success / run_status must
 * never override the persisted run status.
 */
async function onLegacyErecordActivityComplete(input) {
  ...
  var synced = await bridge.syncLegacyRunCompletion({
    legacyRunId: legacy.id,
    workflowRunId: workflowRunId,
    workflowActivityId: activityId,
  })

  var persisted = synced && synced.legacy
  if (!persisted) {
    throw new Error('onLegacyErecordActivityComplete: persisted automation run missing')
  }

  var outcome = classifyRunOutcome(persisted.run_status)
  var runType = persisted.run_type || legacy.run_type
  var activityResult = {
    legacy_run_status: persisted.run_status,
    outcome: outcome,
    error_message: persisted.error_message || null,
  }

  var run = await engine.state.getRun(workflowRunId)
  if (!run) return { skipped: true, reason: 'workflow_run_missing', outcome: outcome }

  if (outcome === RUN_OUTCOME.INTERVENTION) {
    console.log(
      '[epn-migration] Intervention for ' +
        runType +
        ' run=' +
        persisted.id +
        ' workflow=' +
        workflowRunId +
        ' — leaving durable workflow waiting (no ErecordSubmitted / resume)'
    )
    return {
      ok: true,
      outcome: outcome,
  ...
  if (outcome === RUN_OUTCOME.FAILURE) {
    await engine.events.emitEvent({
      ...
      source: 'worker',
      externalId: 'legacy_fail:' + persisted.id,
      payload: {
        legacyRunId: persisted.id,
        runType: runType,
        error: i.errorMessage || persisted.error_message || null,
      },
    })
    await engine.state.updateRun(workflowRunId, {
      status: RUN_STATUS.FAILED,
      error_message: i.errorMessage || persisted.error_message || 'ePN activity failed',
    })
    return { ok: false, outcome: outcome, runId: workflowRunId }
  }

  if (outcome !== RUN_OUTCOME.SUCCESS) {
    throw new Error(
      'onLegacyErecordActivityComplete: cannot advance from run_status=' +
        String(persisted.run_status)
    )
  }

  var eventName =
    runType === 'erecord_prepare'
      ? EVENT_NAMES.ERECORD_PREPARE_COMPLETED
      : runType === 'erecord_submit'
  ...
  await engine.events.emitEvent({
    ...
    companyId: run.company_id,
    source: 'worker',
    externalId: 'legacy_ok:' + persisted.id,
    payload: {
      legacyRunId: persisted.id,
      runType: runType,
      result: i.result || {},
  ...
  await engine.events.emitEvent({
    ...
    companyId: run.company_id,
    source: 'worker',
    externalId: 'activity_ok:' + persisted.id,
    payload: {
      legacyRunId: persisted.id,
      runType: runType,
  ...
  var resumeEpnWorkflow = i.resumeEpnWorkflow
  if (!resumeEpnWorkflow) {
    resumeEpnWorkflow = requireEpnWorkflow().resumeEpnWorkflow
  }
  var resumed = await resumeEpnWorkflow(workflowRunId, {
    engine: engine,
    reason: 'legacy activity complete: ' + runType,
    source: 'worker',
  ...
  return { ok: true, outcome: outcome, run: resumed }
}
