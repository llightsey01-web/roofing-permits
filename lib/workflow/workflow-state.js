...
function createWorkflowState(options) {
  ...
  /**
   * Find paused/waiting runs for webhook correlation.
   * Prefer resume_token prefix = eventName + ':' when eventName provided.
   */
  async function findWaitingRuns(filters) {
    var f = filters || {}
    ...
    var query = supabase
      ...
      .limit(f.limit != null ? f.limit : 25)

    if (f.runId) query = query.eq('id', f.runId)
    if (f.jobId) query = query.eq('job_id', f.jobId)
    if (f.companyId) query = query.eq('company_id', f.companyId)
    if (f.workflowKey) query = query.eq('workflow_key', f.workflowKey)
    if (f.eventName) query = query.like('resume_token', f.eventName + ':%')

    var { data, error } = await query
    if (error) throw new Error('findWaitingRuns: ' + error.message)
    return data || []
  }

  async function listRunsByJob(jobId, options) {
    var o = options || {}
    if (!jobId) throw new Error('listRunsByJob: jobId required')
    ...
    var query = supabase
      ...
      .eq('job_id', jobId)
      .order('created_at', { ascending: false })
      .limit(o.limit != null ? o.limit : 50)

    if (o.workflowKey) query = query.eq('workflow_key', o.workflowKey)
    if (o.status) query = query.eq('status', o.status)

    var { data, error } = await query
    if (error) throw new Error('listRunsByJob: ' + error.message)
    return data || []
  }

  async function findJobIdByProofTransaction(transactionId) {
    if (!transactionId) return null
    var { data, error } = await supabase
      .from('jobs')
      .select('id')
      .contains('job_specs', { proof: { transaction_id: String(transactionId) } })
      .order('updated_at', { ascending: false })
      .limit(1)
      .maybeSingle()
    if (error) throw new Error('findJobIdByProofTransaction: ' + error.message)
    return data && data.id ? data.id : null
  }

  async function findJobIdByErecordPackage(packageId) {
    if (!packageId) return null
    var pack = String(packageId)
    var { data, error } = await supabase
      .from('jobs')
      .select('id')
      .contains('job_specs', { erecord: { package_id: pack } })
      .order('updated_at', { ascending: false })
      .limit(1)
      .maybeSingle()
    if (error) throw new Error('findJobIdByErecordPackage: ' + error.message)
    return data && data.id ? data.id : null
  }
  ...
  return {
    ...
    findWaitingRuns: findWaitingRuns,
    listRunsByJob: listRunsByJob,
    findJobIdByProofTransaction: findJobIdByProofTransaction,
    findJobIdByErecordPackage: findJobIdByErecordPackage,
    updateRun: updateRun,
