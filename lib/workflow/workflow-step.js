...
var { createObservability } = require('./observability.js')
...
function createStepRunner(options) {
  ...
  var observability =
    opts.observability ||
    createObservability({ state: state, logger: logger, artifacts: artifacts })
  ...
  async function executeStep(run, stepDef, context) {
    ...
    async function runOnce(attempt) {
      ...
      await stepLogger.info('step started: ' + stepDef.key, {
        attempt: attempt,
        stepType: stepDef.type,
        stepKey: stepDef.key,
      })

      await observability.logStructured({
        runId: run.id,
        stepId: step.id,
        stepKey: stepDef.key,
        stepType: stepDef.type,
        attempt: attempt,
        level: 'info',
        message: 'observability: step start ' + stepDef.key,
        context: {
          stepType: stepDef.type,
          activityType: stepDef.activityType || null,
          waitForEvent: stepDef.waitForEvent || null,
        },
      })
    ...
    try {
      var output = await retry.withStepRetry(runOnce, {
        ...
        onError: async function (err, attempt, willRetry) {
          ...
          // Capture Playwright evidence on retryable failures when a page is available
          if (ctx.page) {
            try {
              await observability.capturePageEvidence({
                page: ctx.page,
                runId: run.id,
                stepId: step.id,
                stepKey: stepDef.key,
              ...
              console.warn('[observability] attempt failure capture failed:', obsErr.message)
            }
          }
        },
      })

      var durationMs = Date.now() - stepStartedAt
      ...
      if (output && output.waiting) {
        await observability.logStructured({
          ...
          stepKey: stepDef.key,
          level: 'info',
          message: 'observability: step waiting ' + stepDef.key,
          durationMs: durationMs,
          success: true,
          context: {
            waiting: true,
            eventName: output.eventName || null,
            activityId: output.activity && output.activity.id ? output.activity.id : null,
          },
        })
        return {
      ...
      var succeeded = await state.updateStep(step.id, {
        status: constants.STEP_STATUS.SUCCEEDED,
        output: Object.assign({}, output.output || output || {}, { durationMs: durationMs }),
      ...
      await state.appendStepHistory({
        ...
        payload: Object.assign({}, succeeded.output || {}, { durationMs: durationMs }),
      })
      ...
      await observability.logStructured({
        ...
        stepKey: stepDef.key,
        level: 'info',
        message: 'observability: step succeeded ' + stepDef.key,
        durationMs: durationMs,
        success: true,
        context: { stepType: stepDef.type },
      })
      ...
      var failDurationMs = Date.now() - stepStartedAt
      ...
      if (ctx.page) {
        try {
          ...
          console.warn('[observability] step failure capture failed:', obsErr.message)
        }
      }
      ...
      var failed = await state.updateStep(step.id, {
        ...
        completed_at: new Date().toISOString(),
        output: Object.assign({}, step.output || {}, { durationMs: failDurationMs }),
      })
      ...
      await state.appendStepHistory({
        ...
        eventType: 'failed',
        message: err.message,
        payload: { durationMs: failDurationMs },
      })
      ...
      await events.emitEvent({
        ...
        companyId: run.company_id,
        source: 'system',
        payload: { stepKey: stepDef.key, error: err.message, durationMs: failDurationMs },
      })

      await stepLogger.error('step failed: ' + stepDef.key, {
        error: err.message,
      ...
      await observability.logStructured({
        ...
        stepKey: stepDef.key,
        level: 'error',
        message: 'observability: step failed ' + stepDef.key,
        durationMs: failDurationMs,
        success: false,
        context: {
          error: err.message,
