...
async function executeRun(job, runId) {
  try {
    ...
    await supabase.from('automation_runs').update({ run_status: 'needs_review', completed_at: new Date().toISOString() }).eq('id', runId)
    await supabase.from('jobs').update({ job_status: 'needs_review' }).eq('id', job.id)
